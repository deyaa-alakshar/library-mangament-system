...
const RestoreDialog = ({
  ...
  const handleSubmit = async (values: Book) => {
    ...
    try {
      ...
      const promise = await axios.post(
        `${process.env.NEXT_PUBLIC_URL}/books/${values.id}/restore`,
        null,
        {
          headers: {
  ...
  return (
    <Formik onSubmit={handleSubmit} initialValues={book} enableReinitialize>
      {({ submitForm }) => (
        <Form>
          <Dialog.Root open={isOpen} onOpenChange={() => setIsOpen(false)}>
            ...
            <Dialog.Content maxWidth="450px">
              <Dialog.Title>Restore book</Dialog.Title>
